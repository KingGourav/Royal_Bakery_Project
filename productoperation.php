...
<html lang="en">
  ...
  <script src="https://kit.fontawesome.com/9d33041dbc.js" crossorigin="anonymous"></script>
    ...
    <title>Product Operations Form </title>
...
<script>
  ...
  <body>
    ...
    <div class="tom" id="tom1">
        <div class="container">
      <div class="wrap">
  <div><h1>Royal Bakers Shop</h1></div>
    ...
    <div><h5><span>Contact number :-</span>    +91 9889066906</h5></div>
      ...
      <div class="one">
        <h2><i class="fas fa-user-graduate"></i> Total No of Products <span><?php echo ($total); ?></span></h2>
...
$query = "SELECT * FROM product WHERE CONCAT(pid,name,category,price,dis) LIKE '%$se%'";
$data = mysqli_query($conn, $query);
$total = mysqli_num_rows($data);
...
else
{
 ...
 <th scope="col" colspan="2" style="margin-left: 1rem;text-align: center;">Operation</th>
...
<td>".$result['price']."</td>
 <td><a class='' href='productedit.php?a=$result[pid]&b=$result[name]&c=$result[dis]&d=$result[category]&e=$result[price]'>Edit</a></td>

     <td><a class='' href='deleteproduct.php?a=$result[name]'onclick='return checkdelete()'>Delete</a></td>
